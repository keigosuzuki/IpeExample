----------------------------------------------------------------------
-- Vector Plot Importer ipelet for Ipe (MATLAB / Python / R / Julia)
----------------------------------------------------------------------
--
-- Features:
...
label = "Insert Vector Plot"

about = [[
Seamlessly import vector plots (MATLAB / Python / R / etc.) (PDF/IPE) into your Ipe document
...
local function formatGenericPlotText(str)
  ...
  -- Decode [U+XXX] / Unicode representations from pdftoipe
  str = str:gsub("%[U%+[0]*3[bB]1%]", "\\alpha")
  str = str:gsub("%[U%+[0]*3[bB]2%]", "\\beta")
  str = str:gsub("%[U%+[0]*3[bB]3%]", "\\gamma")
  str = str:gsub("%[U%+[0]*3[bB]4%]", "\\delta")
  str = str:gsub("%[U%+[0]*3[bB]5%]", "\\epsilon")
  str = str:gsub("%[U%+[0]*3[bB]6%]", "\\zeta")
  str = str:gsub("%[U%+[0]*[bB]3%]", "\\zeta")   -- matplotlib mathtext cmmi zeta
  str = str:gsub("%[U%+[0]*3[bB]7%]", "\\eta")
  str = str:gsub("%[U%+[0]*3[bB]8%]", "\\theta")
  str = str:gsub("%[U%+[0]*3[bB][bB]%]", "\\lambda")
  str = str:gsub("%[U%+[0]*3[bB][cC]%]", "\\mu")
  str = str:gsub("%[U%+[0]*[bB]5%]", "\\mu")
  str = str:gsub("%[U%+[0]*3[cC]0%]", "\\pi")
  str = str:gsub("%[U%+[0]*3[cC]1%]", "\\rho")
  str = str:gsub("%[U%+[0]*3[cC]3%]", "\\sigma")
  str = str:gsub("%[U%+[0]*3[cC]4%]", "\\tau")
  str = str:gsub("%[U%+[0]*3[cC]6%]", "\\phi")
  str = str:gsub("%[U%+[0]*3[cC]9%]", "\\omega")
  str = str:gsub("%[U%+[0]*3[aA]9%]", "\\Omega")
  str = str:gsub("%[U%+[0]*2212%]", "-")
  str = str:gsub("%[U%+[0]*[bB]1%]", "\\pm")
  str = str:gsub("%[U%+[0]*[bB]0%]", "^\\circ")
  str = str:gsub("%[U%+[0]*[dD]7%]", "\\times")
  str = str:gsub("%[U%+[0]*22[cC]5%]", "\\cdot")
  str = str:gsub("%[U%+[0]*[bB]7%]", "\\cdot")
  str = str:gsub("%[U%+[0]*2264%]", "\\le")
  str = str:gsub("%[U%+[0]*2265%]", "\\ge")
  str = str:gsub("%[U%+[0]*2248%]", "\\approx")
  ...
  str = str:gsub("%(!n%s*=%s*([%d%.]+)%s*rad%/s%)", "($\\omega_n = %1\\,\\mathrm{rad/s}$)")
  str = str:gsub("%(!n%s*=%s*([%d%.]+)%s*rad=s%)", "($\\omega_n = %1\\,\\mathrm{rad/s}$)")
  ...
  str = str:gsub("\\zeta%s*=%s*(%d+)%s*:%s*(%d+)", "$\\zeta = %1.%2$")
  str = str:gsub("\\zeta%s*=%s*([%-%+%.%d]+)", "$\\zeta = %1$")
...
local function importPlot(model, preserveOriginalColors)
  ...
  local file = ipeui.fileDialog(nil, "open", "Select Vector Plot PDF or IPE File (MATLAB / Python / etc.)", filter, nil, nil)
  ...
  for pageNum, p in doc:pages() do
    ...
    model:creation("insert vector plot", group)
    break
  end
  ...
  if model.ui and model.ui.explain then
    ...
    model.ui:explain(string.format("Vector plot inserted (%s, clean TeX applied)", modeMsg))
  end
end

methods = {
  { label = "Insert Vector Plot (Map to Color Palette)", run = function(model) importPlot(model, false) end },
  { label = "Insert Vector Plot (Preserve Original Colors)", run = function(model) importPlot(model, true) end },
}
